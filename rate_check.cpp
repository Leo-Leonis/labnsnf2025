...
// /// @brief (LEO) returns the OLD calibrated values (t_osc dependent,
// FPGA-counts
// /// independent)
...
void rate_check(const int filepath_option = 0, const bool do_print = false,
  ...
  const int n_bins_rate = 50;
  const double min_t = 0;
  const double max_t = 5;
  ...
  TH1D *all_interval_h =
      new TH1D("all_interval_h",
               "All intervals histogram;Interval (s);Entries / (0.1 s^{-1})",
               n_bins_rate, min_t, max_t);

  // histogram of NTF intervals
  TH1D *ntf_interval_h =
      new TH1D("ntf_interval_h",
               "NTF intervals histogram;Interval (s);Entries / (0.1 s^{-1})",
               n_bins_rate, min_t, max_t + 250);
  // n_bins_rate, min_t, 0.05);

  int ev_n, p1, p2, p3; // event number
  double t;             // time of current event
  int n_detections = 0; // number of detections

  double t_past = 0;    // time of past event
  double interval;      // time(current) - time(previous)
  double int_small = 1; // smallest duration
  double int_big = 0;   // biggest duration

  double t_past_ntf = 0;    // time of past NTF event
  double interval_ntf;      // time(current) - time(previous_NTF)
  double int_small_ntf = 1; // smallest NTF duration
  double int_big_ntf = 0;   // biggest NTF duration
  ...
  while (!file.eof()) {
    ...
    interval = t - t_past;
    // std::cout << "DEBUG: interval is " << interval << " s" << '\n'; // debug
    all_interval_h->Fill(interval);

    if (int_small > interval) {
      int_small = interval;
      // std::cout << "LEO_INFO: new smallest interval! (" << std::fixed
      //           << std::setprecision(4) << int_small << " s in ev. " << ev_n
      //           << ")" << '\n'; // debug
    }

    if (int_big < interval) {
      int_big = interval;
      // std::cout << "LEO_INFO: NEW BIGGEST INTERVAL! (" << std::fixed
      //           << std::setprecision(4) << int_big << " s in ev. " << ev_n
      //           << '\n'; // debug
    }

    if (p1 + p2 + p3 == 12285) { // 0. no no no
      ev_count[0]++;
      t_past = t; // final assignement
      continue;   // if no stop then skip
    }

    interval_ntf = t - t_past_ntf;
    // std::cout << "DEBUG: interval is " << interval << " s" << '\n'; // debug
    ntf_interval_h->Fill(interval_ntf);

    if (int_small_ntf > interval_ntf) {
      int_small_ntf = interval_ntf;
      // std::cout << "NTF_LEO_INFO: new smallest interval! (" << std::fixed
      //           << std::setprecision(4) << int_small_ntf << " s in ev. " <<
      //           ev_n
      //           << ")" << '\n'; // debug
    }

    if (int_big_ntf < interval_ntf) {
      int_big_ntf = interval_ntf;
      // std::cout << "NTF_LEO_INFO: NEW BIGGEST INTERVAL! (" << std::fixed
      //           << std::setprecision(4) << int_big_ntf << " s in ev. " <<
      //           ev_n
      //           << '\n'; // debug
    }
  ...
  std::cout << "RESULTS ARE IN: " << '\n'
            << "  int_big = " << int_big << '\n'
            << "  int_small = " << int_small << '\n'
            << "  int_big_ntf = " << int_big_ntf << '\n'
            << "  int_small_ntf = " << int_small_ntf << '\n';
  ...
  /////////////////////// canvas 1 ///////////////////////////////////
  TCanvas *canvas1 = new TCanvas("canvas1", "all intervals", 0, 0, 720, 720);
  // gPad->SetLeftMargin(0.15);
  // gPad->SetRightMargin(0.07);
  // yyn_diff_ch->SetTitleOffset(1.5, "X");
  all_interval_h->SetFillColor(kBlue);
  all_interval_h->Draw();
  std::cout << "all_interval_h->GetMean() = " << all_interval_h->GetMean()
            << "+/-" << all_interval_h->GetMeanError() << '\n'
            << "all_interval_h->GetStdDev() = " << all_interval_h->GetStdDev()
            << " +/- " << all_interval_h->GetStdDevError() << '\n';
  canvas1->SetLogy();

  /////////////////////// canvas 1 ///////////////////////////////////
  TCanvas *canvas2 = new TCanvas("canvas2", "NTF intervals", 0, 720, 720, 720);
  // gPad->SetLeftMargin(0.15);
  // gPad->SetRightMargin(0.07);
  // yyn_diff_ch->SetTitleOffset(1.5, "X");
  ntf_interval_h->SetFillColor(kBlue);
  ntf_interval_h->Draw();
  std::cout << "ntf_interval_h->GetMean() = " << ntf_interval_h->GetMean()
            << "+/-" << ntf_interval_h->GetMeanError() << '\n'
            << "ntf_interval_h->GetStdDev() = " << ntf_interval_h->GetStdDev()
            << " +/- " << ntf_interval_h->GetStdDevError() << '\n';
  canvas2->SetLogy();

  // /////////////////////// canvas 1 ///////////////////////////////////
  // // this will draw the double canvas (the colorful one)

  // TCanvas *canvas1 = new TCanvas("canvas1", "Joint histogram", 0, 0, 1440,
  // 720); canvas1->Divide(2, 1, .001); // divide in in 2 horizonal parts w/
  // 0.001 margin canvas1->cd(1); // go to the first subdivision (the left one
  // in this case)

  // gStyle->SetPalette(kBird); // "kRainBow" is not colourblind friendly!
  // gPad->SetLogy();
  // all_sh->Draw("nostack pmc plc pfc hist p1");
  // gPad->BuildLegend(.53 + 0.09, .62 - 0.05, .9 + 0.09, .93 - 0.05, "",
  // "f");
  // // canvas1->SetFillColor(kRed); // debug

  // canvas1->cd(2); // go to the second subdivision (the right one in this
  // case)
  ...
  //     "all_div_sh",
  //     "Relative presence histogram; Stop time (ns); Ratio / (330
  //     ns^{-1})");

  // // create clone histograms, divide them by total, then add them to the
  // stacked
  ...
  // TCanvas *canvas2 = new TCanvas("canvas2", "\"yes yes no\" time
  // difference", 0,
  //                                720, 720, 720);
  // gPad->SetLeftMargin(0.15);
  // gPad->SetRightMargin(0.07);
  ...
  // TCanvas *canvas3 =
  //     new TCanvas("canvas3", "true positive histogram", 720, 720, 720,
  //     720);
  // gPad->SetLogy();
  ...
  // // NB: here total_h is painted before and tp_h painted after, so there
  // will be
  // // tp_h and the difference between total_h and tp_h, i.e. a histogram
  // stacked
  ...
  if (!do_print) {
    ...
    canvas1->Print("graphs/rate_check/all_intervals.pdf");
    canvas2->Print("graphs/rate_check/ntf_intervals.pdf");
